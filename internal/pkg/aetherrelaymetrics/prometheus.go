...
func (r *Registry) WritePrometheus(w io.Writer) error {
	...
	writeCounter(w, "aetherrelay_requests_total",
	...
	writeCounterFloat(w, "aetherrelay_conversion_duration_seconds_sum",
		"Sum of local conversion processing durations in seconds.", snapshot.conversionDurationSum, conversionKeyLabels)
	writeCounter(w, "aetherrelay_conversion_duration_seconds_count",
		"Number of local conversion processing duration observations.", snapshot.conversionDurationCount, conversionKeyLabels)
	writeCounter(w, "aetherrelay_conversion_features_total",
		"Total bounded ignored or unsupported conversion features.", snapshot.conversionFeatures, conversionFeatureKeyLabels)

	writeCounter(w, "aetherrelay_client_requests_total",
		"Total completed client requests by configured API key ID.", snapshot.clientRequests, clientUsageKeyLabels)
	writeCounter(w, "aetherrelay_client_input_tokens_total",
		"Total input tokens by configured API key ID.", snapshot.clientInput, clientUsageKeyLabels)
	writeCounter(w, "aetherrelay_client_output_tokens_total",
		"Total output tokens by configured API key ID.", snapshot.clientOutput, clientUsageKeyLabels)
	writeCounter(w, "aetherrelay_client_tokens_total",
		"Total input plus output tokens by configured API key ID.", snapshot.clientTokens, clientUsageKeyLabels)
	writeCounter(w, "aetherrelay_usage_store_write_errors_total",
		"Total usage store write errors by phase.", snapshot.usageWriteErr, func(phase string) string { return formatLabels("phase", phase) })
	emitType(w, "aetherrelay_usage_store_query_errors_total", "counter", "Total usage store query errors.")
	_, _ = fmt.Fprintf(w, "aetherrelay_usage_store_query_errors_total %d\n", snapshot.usageQueryErr)
	emitType(w, "aetherrelay_usage_store_query_duration_seconds", "summary", "Usage store query duration in seconds.")
	_, _ = fmt.Fprintf(w, "aetherrelay_usage_store_query_duration_seconds_sum %s\n", formatFloat(snapshot.usageQueryDurationSum))
	_, _ = fmt.Fprintf(w, "aetherrelay_usage_store_query_duration_seconds_count %d\n", snapshot.usageQueryDurationCount)
	emitType(w, "aetherrelay_usage_store_recovered_events_total", "counter", "Total interrupted usage events recovered at startup.")
	_, _ = fmt.Fprintf(w, "aetherrelay_usage_store_recovered_events_total %d\n", snapshot.usageRecovered)
	emitType(w, "aetherrelay_usage_store_checkpoint_errors_total", "counter", "Total usage store checkpoint errors.")
	_, _ = fmt.Fprintf(w, "aetherrelay_usage_store_checkpoint_errors_total %d\n", snapshot.usageCheckpointErr)
	emitType(w, "aetherrelay_usage_store_healthy", "gauge", "Whether the usage store write/query path is healthy (1 healthy, 0 degraded).")
	healthy := 0
	if snapshot.usageHealthy {
		healthy = 1
	}
	_, _ = fmt.Fprintf(w, "aetherrelay_usage_store_healthy %d\n", healthy)
...
func writeSLOWebhookMetrics(w io.Writer, e *SLOEvaluator) {
	...
	emitType(w, "aetherrelay_slo_webhook_dropped_total", "counter",
		"Total SLO webhook batches dropped because the queue was full or the evaluator was closed.")
	_, _ = fmt.Fprintf(w, "aetherrelay_slo_webhook_dropped_total %d\n", dropped)

	emitType(w, "aetherrelay_slo_webhook_queue_length", "gauge",
		"Current number of SLO webhook batches waiting in the send queue.")
	_, _ = fmt.Fprintf(w, "aetherrelay_slo_webhook_queue_length %d\n", queue)

	emitType(w, "aetherrelay_slo_webhook_requests_total", "counter",
		"Total SLO webhook delivery attempts by result (ok, error, non_2xx, canceled).")
	_, _ = fmt.Fprintf(w, "aetherrelay_slo_webhook_requests_total{result=\"ok\"} %d\n", ok)
	_, _ = fmt.Fprintf(w, "aetherrelay_slo_webhook_requests_total{result=\"error\"} %d\n", errN)
	_, _ = fmt.Fprintf(w, "aetherrelay_slo_webhook_requests_total{result=\"non_2xx\"} %d\n", non2xx)
	_, _ = fmt.Fprintf(w, "aetherrelay_slo_webhook_requests_total{result=\"canceled\"} %d\n", canceled)
}
