...
func TestSLOWebhookMetricsExposed(t *testing.T) {
	...
	for _, want := range []string{
		"aetherrelay_slo_webhook_dropped_total",
		"aetherrelay_slo_webhook_queue_length",
		`aetherrelay_slo_webhook_requests_total{result="ok"}`,
		`aetherrelay_slo_webhook_requests_total{result="error"}`,
		`aetherrelay_slo_webhook_requests_total{result="non_2xx"}`,
		`aetherrelay_slo_webhook_requests_total{result="canceled"}`,
	} {
	...
	if !strings.Contains(out, fmt.Sprintf("aetherrelay_slo_webhook_dropped_total %d", e.WebhookDropped())) {
		// 可能中间又 drop 了,至少应 >0
		if !strings.Contains(out, "aetherrelay_slo_webhook_dropped_total ") {
