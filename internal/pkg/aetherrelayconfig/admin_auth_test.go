...
`+minimalProvidersYAML())
	...
	if !cfg.AdminAuth.Enabled || cfg.AdminAuth.Username != "ops-admin" || cfg.AdminAuth.BasePath != "/ops/AetherRelay" || !cfg.AdminAuth.SessionCookieSecure || cfg.AdminAuth.SessionTTLSeconds != 3600 {
...
func TestLoadAdminAuthEnvOverrides(t *testing.T) {
	...
	t.Setenv("AETHERRELAY_ADMIN_AUTH_ENABLED", "true")
	t.Setenv("AETHERRELAY_ADMIN_BASE_PATH", "/secure-admin")
	t.Setenv("AETHERRELAY_ADMIN_USERNAME", "env-admin")
	t.Setenv("AETHERRELAY_ADMIN_PASSWORD_HASH", hash)
	t.Setenv("AETHERRELAY_ADMIN_SESSION_COOKIE_SECURE", "true")
	t.Setenv("AETHERRELAY_ADMIN_SESSION_TTL_SECONDS", "600")
	path := writeConfig(t, "server:\n  listen_addr: 127.0.0.1:8080\n"+minimalProvidersYAML())
	cfg, err := Load(path)
	if err != nil {
		t.Fatal(err)
