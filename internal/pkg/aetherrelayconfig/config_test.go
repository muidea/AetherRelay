...
func TestLoadConfigFileAndEnv(t *testing.T) {
	...
	t.Setenv("AETHERRELAY_LISTEN_ADDR", "127.0.0.1:18080")
...
`), 0o644); err != nil {
	...
	t.Setenv("AETHERRELAY_USAGE_STORE_PATH", "usage.duckdb")
...
func TestLoadStreamIdleTimeoutCanBeDisabledFromEnv(t *testing.T) {
	t.Setenv("AETHERRELAY_STREAM_IDLE_TIMEOUT_SECONDS", "0")
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte(`
server:
...
func TestLoadStreamFirstEventTimeout(t *testing.T) {
	t.Setenv("AETHERRELAY_STREAM_FIRST_EVENT_TIMEOUT_SECONDS", "7")
	cfg, err := Load("")
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadIgnoresAetherRelayDefaultProviderEnv(t *testing.T) {
	// env 不得再注入/覆盖 default_provider 路由语义。
	t.Setenv("AETHERRELAY_DEFAULT_PROVIDER", "openai")
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte(`
providers:
  openai:
...
func TestLoadRejectsInboundEnv(t *testing.T) {
	t.Setenv("AETHERRELAY_INBOUND_API_KEY", "x")
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte(`
providers:
  openai:
...
`), 0o644); err != nil {
	...
	_, err := Load(path)
	if err == nil || !strings.Contains(err.Error(), "AETHERRELAY_INBOUND_API_KEY") {
		t.Fatalf("err = %v", err)
	}
}
...
func TestLoadLogFormatFromConfigAndEnv(t *testing.T) {
	t.Setenv("AETHERRELAY_LOG_FORMAT", "json")
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte(`
server:
  log_format: text
...
func TestLoadRejectsInvalidEnv(t *testing.T) {
	t.Setenv("AETHERRELAY_VERBOSE_LOGGING", "maybe")
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte(`
providers:
  openai:
...
`), 0o644); err != nil {
	...
	if !strings.Contains(err.Error(), "AETHERRELAY_VERBOSE_LOGGING") {
		t.Fatalf("error = %q", err)
	}
}

func TestLoadRejectsInvalidMaxBodyEnv(t *testing.T) {
	t.Setenv("AETHERRELAY_MAX_REQUEST_BODY_BYTES", "invalid")
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte(`
providers:
  openai:
...
`), 0o644); err != nil {
	...
	if !strings.Contains(err.Error(), "AETHERRELAY_MAX_REQUEST_BODY_BYTES") {
		t.Fatalf("error = %q", err)
	}
}
