...
type Config struct {
	...
	// StreamFirstEventTimeout bounds the initial silence of converted SSE
	// streams. It is separate from the inter-event idle timeout so a stalled
	// conversion cannot inherit an unnecessarily long stream window.
...
// StateConfig defines the single local workspace for durable AetherRelay state.
...
type ModelMetadata struct {
	...
	NativeResponsesTools    bool
	NativeResponsesImages   bool
	NativeResponsesDeclared bool
...
const (
	ConversionDirectionResponsesToAnthropic = "responses_to_anthropic"
	ConversionDirectionAnthropicToResponses = "anthropic_to_responses"
...
func Load(path string) (Config, error) {
	cfg := Config{
		...
		StreamFirstEventTimeout:  180 * time.Second,
...
func setTopLevel(cfg *Config, key, value string) error {
	switch key {
	case "usage_file", "AETHERRELAY_USAGE_FILE", "usage_store", "interaction_dir", "interaction_retention":
	...
	case "stream_first_event_timeout_seconds":
		n, err := parseStrictNonNegativeInt(value)
		if err != nil {
			return fmt.Errorf("stream_first_event_timeout_seconds: %w", err)
		}
		cfg.StreamFirstEventTimeout = time.Duration(n) * time.Second
...
func setModelMetadata(cfg *Config, id, key, value string) error {
	...
	case "native_responses_tools":
		b, err := strconv.ParseBool(strings.TrimSpace(value))
		if err != nil {
			return fmt.Errorf("model_metadata.%s.%s: %w", id, key, err)
		}
		info.NativeResponsesDeclared, info.NativeResponsesTools = true, b
	case "native_responses_images":
		b, err := strconv.ParseBool(strings.TrimSpace(value))
		if err != nil {
			return fmt.Errorf("model_metadata.%s.%s: %w", id, key, err)
		}
		info.NativeResponsesDeclared, info.NativeResponsesImages = true, b
	default:
...
// protocol conversion. It deliberately accepts reasoning only through an
// explicit adapter; raw reasoning and thinking payloads have no lossless
// cross-protocol representation.
func ConversionCapabilityUsable(direction string, capability ConversionCapability) bool {
	if capability.Level < 1 || capability.Level > 3 || !capability.Text {
		return false
	}
	if capability.Images || capability.Documents || capability.StructuredOutput || capability.Continuation {
		return false
	}
	if capability.Streaming && capability.Level < 2 {
		return false
	}
	if capability.Tools && capability.Level < 3 {
		return false
	}
...
func conversionReasoningAdapterForDirection(direction, adapter string) bool {
	switch direction {
	case ConversionDirectionResponsesToAnthropic:
		return adapter == ReasoningAdapterResponsesToAnthropicAdaptive
	case ConversionDirectionAnthropicToResponses:
		return adapter == ReasoningAdapterAnthropicToResponsesEffort
	default:
		return false
	}
}
...
func applyEnv(cfg *Config) error {
	if value := os.Getenv("AETHERRELAY_LISTEN_ADDR"); value != "" {
		cfg.ListenAddr = value
	}
	if value := os.Getenv("AETHERRELAY_PORT"); value != "" {
		cfg.ListenAddr = addrFromPort(value)
	}
	if os.Getenv("AETHERRELAY_INBOUND_API_KEY") != "" {
		return fmt.Errorf("AETHERRELAY_INBOUND_API_KEY is not supported; manage client API keys in the state database")
	}
	if os.Getenv("AETHERRELAY_USAGE_FILE") != "" || os.Getenv("AETHERRELAY_USAGE_STORE_PATH") != "" || os.Getenv("AETHERRELAY_CHATGPT_WEB_DATA_DIR") != "" || os.Getenv("AETHERRELAY_INTERACTION_DIR") != "" || os.Getenv("AETHERRELAY_INTERACTION_RETENTION") != "" {
		return fmt.Errorf("legacy state environment variables are not supported; configure the state workspace instead")
	}
	if value := os.Getenv("AETHERRELAY_MAX_REQUEST_BODY_BYTES"); value != "" {
		n, err := parseStrictPositiveInt64(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_MAX_REQUEST_BODY_BYTES: %w", err)
		}
		cfg.MaxRequestBodyBytes = n
	}
	if value := os.Getenv("AETHERRELAY_MAX_UPSTREAM_RESPONSE_BYTES"); value != "" {
		n, err := parseStrictPositiveInt64(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_MAX_UPSTREAM_RESPONSE_BYTES: %w", err)
		}
		cfg.MaxUpstreamResponseBytes = n
	}
	if value := os.Getenv("AETHERRELAY_MAX_STREAM_BYTES"); value != "" {
		n, err := parseStrictPositiveInt64(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_MAX_STREAM_BYTES: %w", err)
		}
		cfg.MaxStreamBytes = n
	}
	if value := os.Getenv("AETHERRELAY_MAX_SSE_LINE_BYTES"); value != "" {
		n, err := parseStrictPositiveInt64(value)
		if err != nil {
	...
	if value := os.Getenv("AETHERRELAY_ARCHIVE_FULL_CONTENT"); value != "" {
		b, err := parseStrictBool(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_ARCHIVE_FULL_CONTENT: %w", err)
		}
		cfg.ArchiveFullContent = b
	}
	if value := os.Getenv("AETHERRELAY_CHATGPT_WEB_ENABLED"); value != "" {
		return fmt.Errorf("AETHERRELAY_CHATGPT_WEB_ENABLED is not supported; the ChatGPT Web account pool is always enabled")
	}
	if value := os.Getenv("AETHERRELAY_CHATGPT_WEB_PROVIDER_ENABLED"); value != "" {
		b, err := parseStrictBool(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_CHATGPT_WEB_PROVIDER_ENABLED: %w", err)
		}
		cfg.ChatGPTWeb.ProviderEnabled = b
		cfg.ChatGPTWeb.providerEnabledConfigured = true
	}
	if value := os.Getenv("AETHERRELAY_CHATGPT_WEB_PRIORITY"); value != "" {
		n, err := parseStrictInt(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_CHATGPT_WEB_PRIORITY: %w", err)
		}
		cfg.ChatGPTWeb.Priority = n
		cfg.ChatGPTWeb.priorityConfigured = true
	}
	if value := os.Getenv("AETHERRELAY_CHATGPT_WEB_REFRESH_ACCOUNT_INTERVAL_MINUTE"); value != "" {
		n, err := parseStrictNonNegativeInt(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_CHATGPT_WEB_REFRESH_ACCOUNT_INTERVAL_MINUTE: %w", err)
		}
		cfg.ChatGPTWeb.RefreshAccountIntervalMinute = n
	}
	if value := os.Getenv("AETHERRELAY_CODEX_OAUTH_ENABLED"); value != "" {
		return fmt.Errorf("AETHERRELAY_CODEX_OAUTH_ENABLED is not supported; the Codex OAuth account pool is always enabled")
	}
	if value := os.Getenv("AETHERRELAY_CODEX_OAUTH_PROVIDER_ENABLED"); value != "" {
		b, err := parseStrictBool(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_CODEX_OAUTH_PROVIDER_ENABLED: %w", err)
		}
		cfg.CodexOAuth.ProviderEnabled = b
		cfg.CodexOAuth.providerEnabledConfigured = true
	}
	if value := os.Getenv("AETHERRELAY_CODEX_OAUTH_PRIORITY"); value != "" {
		n, err := parseStrictInt(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_CODEX_OAUTH_PRIORITY: %w", err)
		}
		cfg.CodexOAuth.Priority = n
		cfg.CodexOAuth.priorityConfigured = true
	}
	if value := os.Getenv("AETHERRELAY_CODEX_OAUTH_REFRESH_ACCOUNT_INTERVAL_MINUTE"); value != "" {
		n, err := parseStrictNonNegativeInt(value)
		if err != nil {
	...
	if value := os.Getenv("AETHERRELAY_VERBOSE_LOGGING"); value != "" {
		b, err := parseStrictBool(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_VERBOSE_LOGGING: %w", err)
		}
		cfg.VerboseLogging = b
	}
	if value := firstEnv("AETHERRELAY_LOG_FORMAT", "LOG_FORMAT"); value != "" {
		cfg.LogFormat = value
	}
	if value := os.Getenv("AETHERRELAY_REQUEST_TIMEOUT_SECONDS"); value != "" {
		n, err := parseStrictPositiveInt(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_REQUEST_TIMEOUT_SECONDS: %w", err)
		}
		cfg.RequestTimeout = time.Duration(n) * time.Second
	}
	if value := os.Getenv("AETHERRELAY_STREAM_IDLE_TIMEOUT_SECONDS"); value != "" {
		n, err := parseStrictNonNegativeInt(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_STREAM_IDLE_TIMEOUT_SECONDS: %w", err)
		}
		cfg.StreamIdleTimeout = time.Duration(n) * time.Second
	}
	if value := os.Getenv("AETHERRELAY_STREAM_FIRST_EVENT_TIMEOUT_SECONDS"); value != "" {
		n, err := parseStrictNonNegativeInt(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_STREAM_FIRST_EVENT_TIMEOUT_SECONDS: %w", err)
		}
		cfg.StreamFirstEventTimeout = time.Duration(n) * time.Second
	}
	if value := os.Getenv("AETHERRELAY_UPSTREAM_BODY_IDLE_TIMEOUT_SECONDS"); value != "" {
		n, err := parseStrictNonNegativeInt(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_UPSTREAM_BODY_IDLE_TIMEOUT_SECONDS: %w", err)
		}
		cfg.UpstreamBodyIdleTimeout = time.Duration(n) * time.Second
	}
	if value := os.Getenv("AETHERRELAY_METRICS_REMOTE_ACCESS"); value != "" {
		b, err := parseStrictBool(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_METRICS_REMOTE_ACCESS: %w", err)
		}
		cfg.MetricsRemoteAccess = b
	}
	...
	if value := os.Getenv("AETHERRELAY_ADMIN_AUTH_ENABLED"); value != "" {
		b, err := parseStrictBool(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_ADMIN_AUTH_ENABLED: %w", err)
		}
		cfg.AdminAuth.Enabled = b
	}
	if value := os.Getenv("AETHERRELAY_ADMIN_BASE_PATH"); value != "" {
		cfg.AdminAuth.BasePath = strings.TrimSpace(value)
	}
	if value := os.Getenv("AETHERRELAY_ADMIN_USERNAME"); value != "" {
		cfg.AdminAuth.Username = strings.TrimSpace(value)
	}
	if value := os.Getenv("AETHERRELAY_ADMIN_PASSWORD_HASH"); value != "" {
		cfg.AdminAuth.PasswordHash = strings.TrimSpace(value)
	}
	if value := os.Getenv("AETHERRELAY_ADMIN_SESSION_COOKIE_SECURE"); value != "" {
		b, err := parseStrictBool(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_ADMIN_SESSION_COOKIE_SECURE: %w", err)
		}
		cfg.AdminAuth.SessionCookieSecure = b
	}
	if value := os.Getenv("AETHERRELAY_ADMIN_SESSION_TTL_SECONDS"); value != "" {
		n, err := parseStrictPositiveInt(value)
		if err != nil {
			return fmt.Errorf("AETHERRELAY_ADMIN_SESSION_TTL_SECONDS: %w", err)
		}
		cfg.AdminAuth.SessionTTLSeconds = n
	}
...
// addrFromPort 将纯端口转为 loopback 地址,避免 AETHERRELAY_PORT=8080 变成 :8080(全网卡)。
...
func normalizeState(state *StateConfig, configPath string) error {
	...
	if state.Database == "" {
		state.Database = "aetherrelay.duckdb"
	}
