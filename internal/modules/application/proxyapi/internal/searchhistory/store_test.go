...
func TestStorePersistsOwnerScopedHistoryAndBoundsListProjection(t *testing.T) {
	store, err := Open(filepath.Join(t.TempDir(), "aetherrelay.duckdb"), "64MB", 1, Config{RetentionDays: 30, MaxItems: 2})
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
...
func TestStoreEnforcesPerOwnerCapacity(t *testing.T) {
	store, err := Open(filepath.Join(t.TempDir(), "aetherrelay.duckdb"), "64MB", 1, Config{RetentionDays: 30, MaxItems: 1})
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
