...
func (s *Proxy) OpenCodexWebsocket(ctx context.Context, request codexresponses.WebsocketOpenRequest) (codexresponses.WebsocketOpenResult, error) {
	...
	for {
		...
		value, sendErr := s.SendEvent(event.NewEventWithContext(upevents.TopicWSOpen, s.ID(), upcommon.UnitID, event.NewHeader(), ctx, upevents.WSOpenCommand{
			...
			ClientIdentity: upevents.ClientIdentity{UserAgent: request.ClientUserAgent, Originator: request.ClientOriginator},
		})).Get()
		if sendErr != nil {
		...
		s.mu.Unlock()
		s.noteCodexTurnState(account.AccountID, fingerprint, request.SessionScope, opened.Headers)
		s.recordCodexTransportCapability(ctx, account.AccountID, accevents.TransportWebsocket, true)
		return codexresponses.WebsocketOpenResult{SessionID: opened.SessionID, Attempt: toCodexHTTPAttempt(opened.Attempt), TurnStateSource: turnStateSource}, nil
	}
}
...
func (s *Proxy) CompleteCodexCompact(ctx context.Context, request codexresponses.Request) (codexresponses.Result, error) {
	...
	for {
		...
		if completed.ErrorClass == "" {
			...
			s.releaseCodexAccount(ctx, account.LeaseID)
			s.recordCodexResult(ctx, account.AccountID, request.Model, true, "", 0, false, "")
			return codexresponses.Result{Body: completed.Body, Headers: toCodexHeaders(completed.Headers), Attempt: toCodexHTTPAttempt(completed.Attempt), TurnStateSource: turnStateSource}, nil
		}
...
func (s *Proxy) StreamCodexResponses(ctx context.Context, request codexresponses.Request, started func(codexresponses.StreamStart) error, emit func([]byte) error) error {
	...
	for {
		...
		if failure.Kind == codexresponses.KindInvalidToken {
			refreshed, refreshErr := s.refreshCodexAccount(ctx, account.AccountID)
			if refreshErr == nil && refreshed.Refreshed {
				err = s.streamCodexOnce(ctx, accevents.AcquireResult{AccountID: refreshed.AccountID, AccessToken: refreshed.AccessToken, AccountIDHeader: refreshed.AccountIDHeader, Proxy: refreshed.Proxy, FingerprintMode: refreshed.FingerprintMode, FingerprintSeed: refreshed.FingerprintSeed}, request, started, func(line []byte) error {
					emitted = emitted || len(line) > 0
					if emit == nil {
						return nil
					}
	...
	for {
		...
		if len(update.Data) > 0 {
			...
			if !clientStarted {
				...
				s.noteCodexTurnState(account.AccountID, fingerprint, request.SessionScope, startedUpstream.Headers)
				clientStarted = true
				if emit != nil {
