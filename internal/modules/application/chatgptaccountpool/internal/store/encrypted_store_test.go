...
func encryptedTestCodec(t *testing.T) *aiproxycredential.Codec {
	t.Helper()
	codec, err := aiproxycredential.New(base64.StdEncoding.EncodeToString(bytes.Repeat([]byte{9}, 32)))
	if err != nil {
		t.Fatal(err)
	}
	return codec
...
func TestOpenRequiresCredentialCodec(t *testing.T) {
	if _, err := Open(filepath.Join(t.TempDir(), "ai-proxy.duckdb"), "256MB", 1, 3, nil); err == nil {
