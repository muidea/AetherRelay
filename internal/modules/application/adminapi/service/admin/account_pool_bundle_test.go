...
func TestAccountPoolBundleImportDoesNotReflectCredentialInStoreFailure(t *testing.T) {
	...
	codex := &codexAccountRuntimeStub{}
	h := NewHandler("", &testRuntime{}).WithChatGPTRuntime(web).WithCodexRuntime(codex)
	body := `{"format":"ai-proxy.account-pool-bundle","schema_version":2,"accounts":[{"account_ref":"acct_01","identity":{"email":"user@example.com"},"slots":{"chatgpt_web":{"access_token":"web-secret","refresh_token":"web-refresh"},"codex_cli":{"access_token":"codex-access","refresh_token":"codex-refresh"}}}]}`
	req := httptest.NewRequest(http.MethodPost, "/admin/api/account-pool-bundle/import", strings.NewReader(body))
	req.RemoteAddr = "127.0.0.1:1234"
	req.Header.Set("X-AI-Proxy-Admin", "1")
	rec := httptest.NewRecorder()
...
func TestAccountPoolBundleImportValidatesAllSlotsBeforeWriting(t *testing.T) {
	...
	codex := &codexAccountRuntimeStub{}
	h := NewHandler("", &testRuntime{}).WithChatGPTRuntime(web).WithCodexRuntime(codex)
	body := `{"format":"ai-proxy.account-pool-bundle","schema_version":2,"accounts":[{"account_ref":"acct_01","slots":{"chatgpt_web":{"access_token":"web-secret","refresh_token":"web-refresh"},"codex_cli":{"credential_type":"wrong","access_token":"codex-secret","refresh_token":"codex-refresh"}}}]}`
	req := httptest.NewRequest(http.MethodPost, "/admin/api/account-pool-bundle/import", strings.NewReader(body))
	req.RemoteAddr = "127.0.0.1:1234"
	req.Header.Set("X-AI-Proxy-Admin", "1")
	rec := httptest.NewRecorder()
...
func TestAccountPoolBundleImportValidatesProxyBeforeWriting(t *testing.T) {
	...
	codex := &codexAccountRuntimeStub{}
	h := NewHandler("", &testRuntime{}).WithChatGPTRuntime(web).WithCodexRuntime(codex)
	body := `{"format":"ai-proxy.account-pool-bundle","schema_version":2,"accounts":[{"account_ref":"acct_01","slots":{"chatgpt_web":{"access_token":"web-access","refresh_token":"web-refresh"},"codex_cli":{"access_token":"codex-access","refresh_token":"codex-refresh","proxy":"file:///tmp/proxy"}}}]}`
	req := httptest.NewRequest(http.MethodPost, "/admin/api/account-pool-bundle/import", strings.NewReader(body))
	req.RemoteAddr = "127.0.0.1:1234"
	req.Header.Set("X-AI-Proxy-Admin", "1")
	rec := httptest.NewRecorder()
...
func TestAccountPoolBundleImportRejectsDuplicateUpstreamAccountID(t *testing.T) {
	...
	codex := &codexAccountRuntimeStub{}
	h := NewHandler("", &testRuntime{}).WithChatGPTRuntime(web).WithCodexRuntime(codex)
	body := `{"format":"ai-proxy.account-pool-bundle","schema_version":2,"accounts":[{"account_ref":"acct-a","slots":{"chatgpt_web":{"account_id":"same-upstream","access_token":"web-a","refresh_token":"refresh-a"}}},{"account_ref":"acct-b","slots":{"chatgpt_web":{"account_id":"same-upstream","access_token":"web-b","refresh_token":"refresh-b"}}}]}`
	req := httptest.NewRequest(http.MethodPost, "/admin/api/account-pool-bundle/import", strings.NewReader(body))
	req.RemoteAddr = "127.0.0.1:1234"
	req.Header.Set("X-AI-Proxy-Admin", "1")
	rec := httptest.NewRecorder()
...
func TestAccountPoolBundleImportRejectsDualSlotWithoutEmail(t *testing.T) {
	...
	codex := &codexAccountRuntimeStub{}
	h := NewHandler("", &testRuntime{}).WithChatGPTRuntime(web).WithCodexRuntime(codex)
	body := `{"format":"ai-proxy.account-pool-bundle","schema_version":2,"accounts":[{"account_ref":"acct_no_email","slots":{"chatgpt_web":{"access_token":"web-access","refresh_token":"web-refresh"},"codex_cli":{"access_token":"codex-access","refresh_token":"codex-refresh"}}}]}`
	req := httptest.NewRequest(http.MethodPost, "/admin/api/account-pool-bundle/import", strings.NewReader(body))
	req.RemoteAddr = "127.0.0.1:1234"
	req.Header.Set("X-AI-Proxy-Admin", "1")
	rec := httptest.NewRecorder()
...
func TestAccountPoolBundleImportRequiresReplaceForDifferentAccountID(t *testing.T) {
	...
	codex := &codexAccountRuntimeStub{}
	h := NewHandler("", &testRuntime{}).WithChatGPTRuntime(web).WithCodexRuntime(codex)
	body := `{"format":"ai-proxy.account-pool-bundle","schema_version":2,"accounts":[{"account_ref":"acct_01","identity":{"email":"operator@example.invalid"},"slots":{"chatgpt_web":{"account_id":"different-upstream","access_token":"new-access","refresh_token":"new-refresh"}}}]}`
	req := httptest.NewRequest(http.MethodPost, "/admin/api/account-pool-bundle/import", strings.NewReader(body))
	req.RemoteAddr = "127.0.0.1:1234"
	req.Header.Set("X-AI-Proxy-Admin", "1")
	rec := httptest.NewRecorder()
	...
	body = `{"format":"ai-proxy.account-pool-bundle","schema_version":2,"replace":true,"accounts":[{"account_ref":"acct_01","identity":{"email":"operator@example.invalid"},"slots":{"chatgpt_web":{"account_id":"different-upstream","access_token":"new-access","refresh_token":"new-refresh"}}}]}`
	req = httptest.NewRequest(http.MethodPost, "/admin/api/account-pool-bundle/import", strings.NewReader(body))
	req.RemoteAddr = "127.0.0.1:1234"
	req.Header.Set("X-AI-Proxy-Admin", "1")
	rec = httptest.NewRecorder()
...
func TestAccountPoolBundleImportSingleSlotDoesNotRequireOtherStore(t *testing.T) {
	...
	body := `{"format":"ai-proxy.account-pool-bundle","schema_version":2,"accounts":[{"account_ref":"acct_web","slots":{"chatgpt_web":{"access_token":"web-access","refresh_token":"web-refresh"}}}]}`
	req := httptest.NewRequest(http.MethodPost, "/admin/api/account-pool-bundle/import", strings.NewReader(body))
	req.RemoteAddr = "127.0.0.1:1234"
	req.Header.Set("X-AI-Proxy-Admin", "1")
	rec := httptest.NewRecorder()
