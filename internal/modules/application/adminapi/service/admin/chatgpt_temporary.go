...
func (h *Handler) startTemporaryTurn(w http.ResponseWriter, r *http.Request, rel string) {
	...
	body, images, attachments, err := decodeTemporaryTurnBody(w, r)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	if strings.TrimSpace(body.Content) == "" && len(images) == 0 && len(attachments) == 0 {
...
func decodeTemporaryTurnBody(w http.ResponseWriter, r *http.Request) (temporaryTurnBody, []tempevents.ImageInput, []chatattachment.File, error) {
	if !strings.HasPrefix(strings.ToLower(r.Header.Get("Content-Type")), "multipart/form-data") {
		var body temporaryTurnBody
		if !decodeAdminBody(w, r, &body) {
			return temporaryTurnBody{}, nil, nil, fmt.Errorf("invalid temporary chat request")
		}
	...
	attachments, err := temporaryTurnAttachments(r.MultipartForm)
	if err != nil {
		return temporaryTurnBody{}, nil, nil, err
	}
	return body, images, attachments, nil
}
...
func temporaryTurnAttachments(form *multipart.Form) ([]chatattachment.File, error) {
	if form == nil {
		return nil, nil
	}
	files := make([]*multipart.FileHeader, 0)
	for _, key := range []string{"attachments", "attachments[]", "files", "files[]"} {
		files = append(files, form.File[key]...)
	}
	...
	result := make([]chatattachment.File, 0, len(files))
	for index, header := range files {
		file, err := header.Open()
		if err != nil {
			return nil, fmt.Errorf("cannot read attachment %d", index+1)
		}
		data, readErr := io.ReadAll(io.LimitReader(file, chatattachment.MaxFileBytes+1))
		closeErr := file.Close()
		if readErr != nil || closeErr != nil {
			return nil, fmt.Errorf("cannot read attachment %d", index+1)
		}
		validated, err := chatattachment.Validate(data, header.Filename, header.Header.Get("Content-Type"))
		if err != nil {
...
func (h *Handler) getTemporaryMessageAttachment(w http.ResponseWriter, r *http.Request, rel string) {
	if h.chatGPT == nil {
		writeError(w, http.StatusServiceUnavailable, "temporary chat unavailable")
		return
	}
	...
	conversationID, messageID, attachmentID := temporaryMessageAttachmentIDs(rel)
	if conversationID == "" || messageID == "" || attachmentID == "" {
		writeError(w, http.StatusBadRequest, "invalid temporary message attachment")
		return
	}
	attachment, err := h.chatGPT.GetTemporaryMessageAttachment(r.Context(), tempevents.GetMessageAttachmentCommand{OwnerID: ownerID, ConversationID: conversationID, MessageID: messageID, AttachmentID: attachmentID})
	if err != nil || len(attachment.Bytes) == 0 {
		writeError(w, http.StatusNotFound, "temporary message attachment not found")
		return
	}
...
func temporaryMessageAttachmentIDs(rel string) (conversationID, messageID, attachmentID string) {
	parts := strings.Split(strings.Trim(rel, "/"), "/")
	if len(parts) != 8 || parts[0] != "api" || parts[1] != "chatgpt" || parts[2] != "temporary-conversations" || parts[4] != "messages" || parts[6] != "attachments" {
		return "", "", ""
	}
	return strings.TrimSpace(parts[3]), strings.TrimSpace(parts[5]), strings.TrimSpace(parts[7])
}
...
func writeTemporaryChatError(w http.ResponseWriter, err error) {
	...
	case strings.Contains(lower, "required") || strings.Contains(lower, "invalid") || strings.Contains(lower, "limit") || strings.Contains(lower, "exceeds") || strings.Contains(lower, "image") || strings.Contains(lower, "attachment") || strings.Contains(lower, "file") || strings.Contains(lower, "supported"):
