...
func TestHandlerServesProjectAdminPageAndMasksAPIKey(t *testing.T) {
	...
	for _, marker := range []string{
		...
		`id="tcAttach" title="添加图片" aria-label="添加图片"`, `<svg viewBox="0 0 24 24" aria-hidden="true">`, ".tc-citation", "function normalizeTemporaryContent(value)", "function renderTemporaryContent(value)", "renderTemporaryContent(content)", "normalizeTemporaryContent(msg.content)",
	} {
