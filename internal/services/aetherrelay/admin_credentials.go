...
func runAdminSetCredentials(args []string) int {
	fs := flag.NewFlagSet("AetherRelay admin set-credentials", flag.ContinueOnError)
	fs.SetOutput(io.Discard)
	username := fs.String("username", "", "Admin username")
	configPath := fs.String("config", os.Getenv("AETHERRELAY_CONFIG"), "config file path")
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 || strings.TrimSpace(*username) == "" {
		fmt.Fprintln(os.Stderr, "usage: AetherRelay admin set-credentials --username <username> [--config <config.yaml>]")
		return 2
	}
	...
	fmt.Fprintf(os.Stdout, "Admin credentials saved in %s. Restart AetherRelay to apply them.\n", path)
	return 0
}
