...
function harness() {
  ...
  for (const name of ['formatNumber', 'compact', 'usageCacheRate', 'usageCacheHint', 'svgBars', 'svgStacked', 'renderCharts', 'renderKeyTable', 'eventStatusMeta', 'usageUpstreamMeta', 'usageParams', 'loadUsage', 'loadEvents', 'showUsageEvent']) {
...
test('dashboard, chart, key table and events render server cache statistics', async () => {
  ...
  const event = {...usage, api_key_id: 'key-a', event_id: 'event-a', provider: 'codexoauth', model: 'gpt-5.6-sol', operation: 'responses', conversion_mode: 'codex_oauth_responses', http_status: 503, outcome: 'provider_unavailable'};
...
test('usage tables reserve stable widths and clip long cells', () => {
  assert.match(html, /\.usage-key-table\{min-width:1200px\}/);
  assert.match(html, /\.usage-event-table\{min-width:1590px\}/);
