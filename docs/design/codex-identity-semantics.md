...
>
> 实现基线：AetherRelay `13.3.0` 工作树（2026-09-21）
...
13.3.0 修复消息树将工具参数对象误计为协议内容块的问题，并增加结构超限错误合同；本文身份派生、UA/Originator、缓存隔离与账号调度不变。现场 001454–001456 为本地转换阶段拒绝，未访问上游，不能将其解释为上游身份不匹配。
