...
导入在调用任一账号 Store 前先完成整包预检和规范化；只要文件存在格式错误或文件内冲突，就不会写入任何一个 Store。预检会校验 `account_ref` 唯一、每个账号至少一个槽位、槽位 `credential_type` 与槽位名称一致、凭据字段完整、代理地址可接受、双槽位邮箱一致，以及不能重复使用同一槽位凭据或同类型上游 `account_id`。同一邮箱允许出现在多个 `account_ref`，以保留同邮箱下的不同账号或工作区；目标匹配仍须由槽位 `account_id` 消除歧义。Token 只用于校验和传递，错误响应不会回显其内容。
