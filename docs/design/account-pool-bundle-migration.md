...
- `codex_cli.fingerprint_mode` 只允许 `off/device/session/full`；缺失视为 `off`。它是显式的账号级行为设置，随凭据迁移但不包含任何派生 ID或私有 fingerprint seed；因此导入为新账号时会生成新的设备身份。数据库级加密归档恢复会保留内部 seed。
