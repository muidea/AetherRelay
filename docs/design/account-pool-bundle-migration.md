...
- `codex_cli.fingerprint_mode` 只允许 `off/scoped`；缺失视为 `scoped`。它是账号级行为设置，随凭据迁移但不包含任何派生 ID 或私有 fingerprint seed；因此导入为新账号时会生成新的账号投影身份。数据库级加密归档恢复会保留内部 seed。
...
1. 先按非空 `identity_key` 做唯一的一对一跨槽位匹配；管理列表缺少身份摘要时，从完整凭据中的 `account_id`/邮箱生成回退摘要。两侧邮箱同时非空时还必须一致；两侧都没有邮箱时不组成双槽位账号。
2. 对尚未匹配且有邮箱的槽位，再按 `trim + strings.ToLower(email)` 匹配，但仅当该邮箱剩余恰好一个 ChatGPT Web 和一个 Codex 槽位时才合并。
3. 同一邮箱存在多个同类型槽位或其它多对多歧义时，不猜测配对关系、不覆盖也不阻断导出；每个未匹配槽位使用独立 `account_ref` 完整保留。因此不同 `account_ref` 可以具有同一邮箱。
4. `account_ref` 使用随机生成的 `acct_` 引用，不能使用邮箱，也不能使用任一上游账号 ID；输出顺序保持源账号池顺序稳定。

该分组规则仅用于整体迁移，不改变运行时 `identity_key` 的生成和路由行为。

## 导入匹配规则

导入在调用任一账号 Store 前先完成整包预检和规范化；只要文件存在格式错误或文件内冲突，就不会写入任何一个 Store。预检会校验 `account_ref` 唯一、每个账号至少一个槽位、槽位 `credential_type` 与槽位名称一致、凭据字段完整、代理地址可接受、双槽位邮箱一致，以及最终生效槽位不能重复使用同一凭据。同一槽位类型中，非空且完全相同的 `account_id` 视为同名，按文件顺序由后一个槽位覆盖前一个槽位；凭据唯一性在覆盖完成后校验。同一邮箱不属于同名覆盖键，允许出现在多个 `account_ref`，以保留同邮箱下的不同账号或工作区。Token 只用于校验和传递，错误响应不会回显其内容。

每个槽位仍交给现有对应 Store 的导入流程，两个槽位不会写入同一张账号表。统一账号关系按以下顺序恢复：

1. 文件内 `account_ref` 将同一文件中的槽位归为一组；同类型 `account_id` 重名时先执行后者覆盖。
2. 目标实例已有账号优先按槽位 `account_id` 生成的身份摘要精确匹配；明确提供相同 `account_id` 的凭据允许更新同一槽位。
3. 仅在未提供 `account_id` 时，才按槽位邮箱、再按 `identity.email` 回退匹配；邮箱必须唯一命中，且不会因为目标摘要来自另一个上游 `account_id` 而误报冲突。
4. 槽位显式提供的上游 `account_id` 无法精确命中时，默认作为同邮箱下的独立账号或工作区新增；只有显式 `replace=true` 才允许按唯一邮箱替换现有槽位。
...
- 两个槽位分别提供启用开关；关闭后禁止对应凭证参加新的业务路由调度，另一个槽位保持独立。禁用期间仍执行 ChatGPT Web 额度与必要的凭据续期，以及 Codex 上游用量刷新；刷新结果更新账号快照但保留禁用状态。重新启用会将对应槽位恢复为正常状态，再由最新模型、额度和冷却规则决定实际可用性。
- 额度或用量刷新任务完成后，管理端重新读取对应 Store 的账号列表，并用最新快照重算槽位展示和统一账号统计；“可用图片额度”只统计当前可路由槽位，禁用槽位保留的最新额度会在重新启用后纳入统计。
...
分槽位导入成功后，页面重新加载两个 Store 的列表并重新计算统一账号行。Web 展示聚合采用以下候选键：

1. 槽位 `identity_key`；
2. 槽位邮箱的规范化值（去首尾空格并转小写）；
3. 槽位本地 `id`（仅用于避免无邮箱账号碰撞）。

页面与导出使用相同的多重性安全规则：先按唯一 `identity_key` 配对，再对剩余槽位做唯一邮箱配对。同一邮箱存在多个同类型槽位时不会用后一个覆盖前一个；无法唯一配对的槽位全部显示为独立行。邮箱缺失、不一致或存在多对多歧义时不会强行合并。
