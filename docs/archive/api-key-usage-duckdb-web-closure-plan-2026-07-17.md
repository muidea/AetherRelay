...
默认模式下，接口复用 `<admin_base_path>`（默认 `/admin`）的 loopback-only 访问控制和安全响应头；启用 `admin_auth_enabled` 后，接口改由 Admin 登录会话保护，并可远程访问（生产环境推荐 HTTPS）。
...
- `<admin_base_path>/api/usage/*` 在默认模式仅允许 loopback；启用 `admin_auth_enabled` 后要求有效 Admin 会话。
...
- [x] CSV 导出可用且不泄密。
- [x] 页面保持轻量、响应式；默认模式 loopback-only，安全登录模式由会话保护。
