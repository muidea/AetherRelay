...
> 状态：historical design record；当前行为以配置参考、自动化测试和 [强制联网搜索](chatgpt-web-search-design-2026-08-03.md) 为准。
>
> 范围：管理台的服务端保留多轮对话与历史恢复；当前实现经有效 Provider 候选链执行。
...
当前新建会话固定请求模型而不固定账号；每轮通过有效 Provider 候选链执行。旧版账号固定会话仅保留恢复兼容，不再作为新建路径。ChatGPT Web 的旧续聊路径仍使用持久化上游锚点；Provider-neutral 路径会安全地提交已持久化的有界历史。

不实现：research / 深度研究、浏览器插件、持久搜索线程、复杂工具循环或跨账号恢复旧版上游会话。临时对话可按轮启用一次受限联网搜索；搜索不接受图片或文件附件，也不承诺多轮工具语义。research 相关模型不得以普通聊天模型身份暴露给管理台或 `/v1/models`。
