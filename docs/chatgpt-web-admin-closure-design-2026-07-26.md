...
导入对话框只支持当前后端接收的 access token 集合：可将文本按换行/逗号拆分，也可粘贴 JSON 数组（字符串或含 `access_token` 的对象，例如已有 `accounts.json`）；前端提取、去空白和去重后仅提交 `tokens`。`source_type` 可由下拉框选择“手工导入”或留空；UI 不提供 CPA/Sub2API 选项。关闭、取消或导入完成后必须清空原始输入。
