...
创建、启停、轮换、删除客户端 API Key；管理端生成的 Key 以 SHA-256 摘要形式写回配置。
...
- 统一账号状态按最严重的有效槽位聚合：任一槽异常则为“异常”，否则 ChatGPT Web 限流则为“限流”，全部已配置槽均禁用才为“禁用”，其余存在正常槽则为“正常”。页面同时保留各凭据槽原始状态，并分别统计正常/限流、异常/禁用账号数以及所有正常或限流 ChatGPT Web 槽的可用图片额度总和。
