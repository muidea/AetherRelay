...
`supported_endpoints` 只表示模型至少有一个已配置或已发现的目录候选具备该客户端路径合同，不包含请求期健康度和熔断过滤。例如 Anthropic Provider 声明原生 `messages` 时，模型可以同时显示 `/v1/messages` 与经协议转换的 `/v1/chat/completions`；ChatGPT Web 模型必须由上游目录明确发布 `text_generation` 才派生文本、Responses 与搜索入口，明确发布 `image_generation` 才派生图片生成和编辑入口。旧快照缺少 capability 时只保留文本能力，避免把所有模型都宣称为图片模型。系统信息中的“开放 API 端点”是实例级路由清单，需与模型的 `supported_endpoints` 取交集后再发起请求；即使目录命中，全部候选临时熔断时仍会返回 `provider_unavailable`。
...
- 每个已接受请求先写 DuckDB `started` 事件，随后结算为 `completed`；流式真实结束态用 outcome（`success`、`client_canceled`、`first_event_timeout`、`idle_timeout`、`upstream_truncated`、`upstream_failed` 等）统一写入 DuckDB / Prometheus / `metadata.json`。客户端取消不计为上游故障。
...
- 统一账号池可分别启用或禁用 ChatGPT Web、Codex CLI 凭据槽。禁用状态持久化并立即阻止该凭证进入新的文本、图片和 Responses 路由调度，不影响同一统一账号中的另一个凭据槽。ChatGPT Web 额度与必要的凭据续期、Codex 上游用量在禁用期间继续刷新，最新快照会同步到账号列表与统一账号统计且不会自动启用账号；重新启用后，实际路由仍服从模型快照、最新额度、冷却与并发限制。

- ChatGPT Web：导入纯 access token 或完整 OAuth 凭据、批量刷新、删除、OAuth 导入；不提供 ChatGPT Web 槽位单独导出。账号行提供「重新认证」，OAuth session 绑定稳定本地 ID；未显式选择账号时仍按上游 `account_id`、缺失时按唯一邮箱收敛 token 轮换，不会因新 access token 生成重复记录。同邮箱但不同 `account_id` 的 workspace 保持分离。重新认证保留本地 ID、创建时间、统计与代理，恢复系统异常状态并清理旧凭据错误，但不覆盖显式禁用。新增或重新认证成功后会立即提交该账号的后台信息刷新，管理页静默轮询并更新图片额度和账号状态；该后续刷新失败不回滚已写入的 OAuth 凭据。账号池入口本身决定凭据类型，不强制要求冗余 `credential_type` 字段；所有 ChatGPT Web 凭据来源采用同一刷新语义，手工操作统一标为「刷新额度/凭据」，只处理所选账号并尊重显式禁用，无可刷新账号时显示明确错误而非成功 `0/0`。手工刷新若同时续期 OAuth 凭据，即使旧 access token 仍可读取账号信息，凭据续期失败也会计入失败并明确显示。账号刷新兼容上游图片额度字段的 snake_case / camelCase 变体和常见图片能力名称，并明确展示成功/失败数量。未返回可识别额度时刷新失败并保留原额度，不会静默写成 `0`；明确返回额度耗尽或图片能力被阻断时才写入 `0`。单账号上游刷新限制为 45 秒，进度读取独立于长耗时账号命令。只读展示文本/生图模型冷却、最近凭据刷新状态；支持「同步模型」。
- Codex OAuth：导入完整凭据、刷新、删除、PKCE OAuth 导入、批量刷新所选用量、同步模型并轮询进度；不提供 Codex 槽位单独导出。账号行提供「重新认证」，OAuth session 绑定稳定本地 ID并复用账号既有代理；未显式选择账号时按上游 `account_id`、缺失时按唯一邮箱自动收敛全量 token 轮换。同邮箱但不同 `account_id` 的 workspace 保持分离。重新认证保留本地 ID、创建时间、统计、代理、指纹设置及已选客户端 profile，恢复系统异常状态并清理旧刷新、冷却、模型与用量错误，但不覆盖显式禁用。账号池入口本身决定凭据类型，不强制要求冗余 `credential_type` 字段。每个账号可设置 `fingerprint_mode=off/scoped`，默认 `scoped`；统一账号池与独立 Codex 列表均始终显示当前模式并可直接切换，显式 `off` 不会被隐藏。`scoped` 以账号随机 seed 和下游 LogicalConversation/LogicalThread 派生隔离的上游 Installation/Session/Thread；同时从实际选中账号的客户端请求中原子观察 User-Agent/Originator，按已验证 family 与版本单调选择账号级 profile，供推理、模型发现、用量查询和 refresh 共用。私有 seed 与 profile 随数据库归档恢复，但不进入直接凭据或普通账号池导出；显式替换槽位凭据时重新生成/清除。账号切换只替换账号物理投影，不改变请求级 Turn、Window、缓存身份、正文或工具顺序。refresh token 使用当前 Codex CLI 的 JSON 请求合同，并区分明确的过期、重复使用、撤销与暂时性刷新错误，不再把所有 HTTP 400 归为 `invalid_token`。管理页「刷新凭据」表示强制续期 OAuth 凭据，成功后自动提交所选账号的模型同步和上游用量刷新，并明确提示该联动语义；导入和 OAuth 完成也触发相同的账号范围同步，不进行全量账号扫描。凭据刷新健康与当前 access token 路由健康独立展示，刷新失败不会否定随后成功的模型、用量或 Responses 鉴权；成功结果可恢复系统异常状态，但不会覆盖显式禁用。显式用量刷新和模型同步都可重试异常账号；用量请求遇到 `401` 时会刷新凭据一次。两类任务都会将未进入候选的所选账号计入失败和总数，不显示为成功 `0/0`。用量解析兼容 snake_case / camelCase，使用 `used_percent_known` 区分未知值与 `0%`。响应没有任何有效用量窗口时刷新失败并保留最近一次成功快照，页面显示失败数量和安全错误类别。账号统计与 ChatGPT Web 统一为四列（总数、正常、异常/禁用、可路由），具体用量限制保留在账号行展示；同时展示模型缓存、发现进度/退避、上游用量窗口（套餐、`used_percent`、恢复时间）、模型冷却与额度耗尽状态。
- 非 Codex SDK、OpenCode、Claude Code 等客户端可正常进入上述数据面，但其身份只有在符合已验证 Codex 原子 profile 时才可影响 `scoped` 账号；缺失或异常身份不会拒绝业务请求。完全无会话信号的请求使用请求级 Session/cache identity，避免同一 Key 与模型下的无关调用意外共享上下文。
- Codex Responses 的账号切换耗尽后会返回最后一个真实上游失败及安全 HTTP 状态，不再把 401、403、429 或 5xx 覆盖成泛化的 `provider_unavailable`；全部匹配凭据的 refresh token 永久失效时返回 503 `upstream_authentication_required`、`authentication_error` 与 `retryable=false`，并在管理页标记“凭据永久失效 · 需重新认证”。该终态停止凭据参与路由，但用量刷新仍会读取并同步可获得的最新快照；模型目录保留最后已知 membership 只用于生成可操作的鉴权终态，不会对失效凭据发起推理。不会记录上游响应正文或任何账号凭据。
- Web 端保留 ChatGPT Web 和 Codex 两个独立导入入口，并在统一账号列表工具栏提供始终可见的“账号池迁移 ▾”分组入口，集中管理整体包导入与导出；整体包是唯一账号池导出格式。号池列表不再提供“搜索邮箱或本地 ID”输入框；列表顶部工具栏相对页面头部粘滞浮动，随视图滚动保持操作按钮可达。整体包导入先完成整包预检，文件内冲突返回安全的冲突列表且不写入任一槽位；预检通过后按 `account_id`/邮箱建立目标匹配，只有显式 `replace=true` 才替换同邮箱的不同上游账号，跨 Store 失败时明确标记部分成功。分槽位导入和整体包导入均在成功后重新计算统一账号行。分槽位导入同时接受单条凭据对象 `{...}`、对象数组 `[...]` 和 `{accounts:[...]}` 包装；ChatGPT Web 额外支持粘贴纯 access token。分槽位入口可一次选择最多 20 个文件并合并为一个批量请求；ChatGPT Web 保持任一文件无效即不提交，Codex 则整份忽略无法读取、解析、缺少必要凭据或凭据类型错误的文件，再提交其余有效文件。选择集、请求体和有效账号批次分别受 1 MiB、1 MiB、1000 个账号限制。文件与粘贴内容不能同时使用，提交后清除浏览器内存引用。
...
- **图片**：上游目录对具体模型明确发布 `image_generation` 后，该模型才在 `/v1/models` 中声明 `/v1/images/generations` / `/v1/images/edits` 并参与图片路由；不会因名称包含 `image` 或本地静态别名推断能力。成功响应中的图片字节按认证得到的 `api_key_id` 存储，原始 API Key 不进入路径、数据库或日志。ChatGPT Web 返回认证后的图片 URL 时，内部会下载并验证栅格 bytes；明确 `size=WIDTHxHEIGHT` 会本地规范化为精确 PNG，`auto` 保留上游尺寸。ChatGPT Web 的 `response_format=b64_json` 直接返回图片内容，`response_format=url` 返回一小时有效的 AetherRelay 签名 URL；原生 Images Provider 返回的 URL 仍遵循对应上游的生命周期。签名密钥由部署凭证主密钥按独立用途派生，客户端无法使用自己的 API Key 伪造。读取时由 AetherRelay 校验签名、完整路径、有效期和当前启用的凭据作用域，凭据禁用、撤销或轮换后已签发 URL 立即失效；若凭据在生图执行期间失效，标准请求返回明确鉴权错误，显式允许格式降级的扩展客户端才会收到内联 `b64_json`，并通过正文与响应头获知实际格式；多图内联共享整次响应大小预算。删除 Client Key 会先持久化待删除状态并禁用，再等待全部在途外部请求完成并幂等清理图片作用域，失败或重启后保持待删除状态，可重试 DELETE 继续清理。SVG/vector 不支持；上游仅有不可下载或不可解码内容时请求失败，不声称已归档。
...
- 日志与归档脱敏 `Authorization` / `X-API-Key` / `Cookie` / `Set-Cookie` / `WWW-Authenticate`，以及 Codex account/session/thread/turn/fingerprint 等身份 Header；请求与响应两个方向使用同一名单。
